...
export default function detectEthereumProvider<T = BitkeepEthereumProvider>({
    ...
    return new Promise((resolve) => {
        console.log("window.bitkeep.ethereum=>>>>>>>>>>>>>>>>>>>>>>>", window.bitkeep.ethereum)
        ...
        function handleEthereum() {
            ...
            if (window.bitkeep.ethereum && (!mustBeMetaMask || window.bitkeep.ethereum.isBitkeep)) {
                resolve(window.bitkeep.ethereum as unknown as T);
            } else {

                const message = mustBeMetaMask && window.bitkeep.ethereum
