...
export class Okex extends Connector {
  ...
  private async isomorphicInitialize(): Promise<void> {
    ...
    return (this.eagerConnection = import('./OkexEthereumProvider').then(async (m) => {
      ...
      if (provider) {
        ...
        if (this.provider.providers?.length) {
          this.provider = this.provider.providers.find((p) => p.isOKExWallet) ?? this.provider.providers[0]
        }
