...
const TokenSelection = ({token, gasToken, onSetGasToken}: { token: Token, gasToken: Token | undefined, onSetGasToken: (token: Token) => void }) => {
    const { wallet } = useSmartAccountContext()
    const balance = useCurrencyBalance(wallet?.address, token)

    return (
        <TokenWrapper gap="10px" jus="space-between" onClick={() => onSetGasToken(token)} isActived={gasToken?.address === token.address}>
            ...
            <Row al="center" className="to">{balance || '0.00'}</Row>
        </TokenWrapper>
    )
}
            ...
            <ModalWrapper>
                ...
                <ModalBody>
                    <div className="subtitle">
                        Choose one of tokens bellow to pay gas fee for you smart account transaction using Paymaster.
                    </div>
                    <div>Select a token: </div>
                    {
                        state?.chainId && GAS_TOKEN[state.chainId].map((token, index) => {
                            return(
                                <TokenSelection key={index} token={token} gasToken={gasToken} onSetGasToken={setGasToken} />
                            )
                        })
                    }
                    <PrimaryButton
