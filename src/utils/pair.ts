...
import { mul, div, add, sub, divNumberWithDecimal, fixNum, mulNumberWithDecimal } from './math'
...
export class Pair {
    ...
    calcShareOfPool(
        ...
        amount0 = isSortedTokens(token0, token1) ? mulNumberWithDecimal(amount0, token0.decimals) : mulNumberWithDecimal(amount1, token1.decimals)
        amount1 = isSortedTokens(token0, token1) ? mulNumberWithDecimal(amount1, token1.decimals) : mulNumberWithDecimal(tempAmount, token0.decimals)
        if (this.reserve0 && this.reserve1 && this.reserveLp) {
            ...
            const addedLp = Number(lpShare0) < Number(lpShare1) ? lpShare0 : lpShare1
            console.log({ addedLp })
            const shareOfLp = mul(div(addedLp, add(addedLp, this.reserveLp)), 100) //%
